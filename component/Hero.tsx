...
export default function AyurvedaBanner() {
  ...
  return (
    <div style={{ position: "relative", width: "100%", minHeight: isMobile ? 660 : 520, overflow: "hidden", display: "flex", alignItems: "center", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{
        ...
        background: isMobile
          ? "linear-gradient(160deg,rgba(8,22,12,0.72) 0%,rgba(8,22,12,0.60) 55%,rgba(8,22,12,0.30) 100%)"
          : "linear-gradient(110deg,rgba(8,22,12,0.54) 0%,rgba(8,22,12,0.36) 45%,rgba(8,22,12,0.12) 72%,transparent 100%)",
      }} />
      ...
      {/* Content */}
      <div style={{ position: "relative", zIndex: 4, padding: isMobile ? "108px 20px 56px 24px" : "100px 44px 52px 56px", maxWidth: 860, width: "100%" }}>

        {/* 1. Tag */}
        <div style={anim(0.1)}>
          <span style={{ fontSize: isMobile ? 9 : 10, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--vk-lime)", padding: isMobile ? "5px 10px" : "5px 14px", borderRadius: 4, border: "1px solid rgba(232,184,75,0.4)", background: "rgba(200,150,12,0.12)", display: "inline-block", marginBottom: isMobile ? 14 : 20 }}>
            {copy.tag}
          </span>
        </div>
        ...
        {/* 2. Headline */}
        <div style={anim(0.2)}>
          <h1 style={{ fontFamily: "Georgia,'Playfair Display',serif", fontSize: isMobile ? (language === "ta" ? "clamp(1.65rem,7.5vw,2.25rem)" : "clamp(2.1rem,10vw,3rem)") : language === "ta" ? "clamp(1.3rem,3.8vw,2.9rem)" : "clamp(1.5rem,4.5vw,4rem)", fontWeight: 900, lineHeight: 1.1, color: "#fff", textShadow: "0 2px 28px rgba(0,0,0,0.55)", margin: 0, marginBottom: isMobile ? 12 : 16 }}>
        ...
        {/* 3. Subtitle */}
        <div style={anim(0.3)}>
          <p style={{ fontSize: isMobile ? 14.5 : 17, color: "rgba(255,255,255,0.84)", lineHeight: 1.65, margin: 0, marginBottom: isMobile ? 18 : 24, maxWidth: 560, borderLeft: "2px solid rgba(232,184,75,0.45)", paddingLeft: isMobile ? 10 : 14 }}>
            {copy.subtitle[0]}<br />{copy.subtitle[1]}
          </p>
        </div>

        {/* 4. Active label + Condition pills */}
        <div style={anim(0.4)}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: "var(--vk-lime)", transition: "opacity 0.4s ease", opacity: labelVisible ? 1 : 0 }}>
              {copy.conditions[active]}
            </span>
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: isMobile ? 6 : 8, marginBottom: isMobile ? 20 : 26 }}>
            {conditions.map((_, i) => (
              <span
                key={copy.conditions[i]}
                onClick={() => handlePillClick(i)}
                style={{
                  fontSize: isMobile ? 12 : 13.5, fontWeight: i === active ? 700 : 400,
                  padding: isMobile ? "5px 12px" : "6px 16px",
                  borderRadius: 99, cursor: "pointer",
                  border: `1px solid ${i === active ? "rgba(232,184,75,0.7)" : "rgba(255,255,255,0.18)"}`,
                  background: i === active ? "rgba(200,150,12,0.22)" : "rgba(255,255,255,0.06)",
                  color: i === active ? "var(--vk-lime)" : "rgba(255,255,255,0.88)",
        ...
        {/* 5. CTA — Book + Call */}
        <div style={anim(0.5)}>
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: isMobile ? 10 : 14, marginBottom: isMobile ? 18 : 22 }}>
            <button
              onClick={() => window.dispatchEvent(new CustomEvent("vk-open-booking"))}
              className="rv2-cta-link"
              style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, color: "#fff", fontWeight: 700, fontSize: isMobile ? 13 : 14, padding: isMobile ? "13px 20px" : "13px 28px", borderRadius: 50, cursor: "pointer", letterSpacing: "0.04em", textTransform: "uppercase", width: isMobile ? "100%" : undefined }}
            >
            ...
            <a href="tel:9996660102" style={{ display: "flex", alignItems: "center", gap: 10, color: "rgba(255,255,255,0.92)", fontSize: isMobile ? 13 : 14, fontWeight: 500, textDecoration: "none", width: isMobile ? "100%" : undefined }}>
              <span style={{ width: 36, height: 36, borderRadius: "50%", border: "1px solid rgba(232,184,75,0.4)", background: "rgba(200,150,12,0.14)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        ...
        {/* 6. Price pill + trust badges — grouped at bottom */}
        <div style={anim(0.6)}>
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: isMobile ? 8 : 12, marginBottom: isMobile ? 12 : 14 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, fontSize: isMobile ? 13 : 14, fontWeight: 700, padding: isMobile ? "7px 14px" : "8px 20px", borderRadius: 50, background: "rgba(232,184,75,0.15)", border: "1px solid rgba(232,184,75,0.4)", color: "var(--vk-lime)" }}>
              {copy.consultation}&nbsp;<span style={{ fontSize: isMobile ? 18 : 20, fontFamily: "Georgia,serif" }}>₹150</span>
              <span style={{ fontSize: 11, opacity: 0.75 }}>{copy.only}</span>
            </div>
            <span style={{ fontSize: isMobile ? 12 : 13, color: "rgba(255,255,255,0.88)", fontWeight: 500 }}>· {copy.limitedSlots}</span>
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: isMobile ? 6 : 8 }}>
            {copy.badges.map((b) => (
              <div key={b} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: isMobile ? 11.5 : 13, fontWeight: 500, color: "rgba(255,255,255,0.82)", padding: isMobile ? "4px 9px" : "5px 12px", borderRadius: 4, border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.06)" }}>
                <span style={{ width: 5, height: 5, borderRadius: "50%", background: "var(--vk-lime)", display: "inline-block", flexShrink: 0 }} />
                {b}
              </div>
            ))}
          </div>
